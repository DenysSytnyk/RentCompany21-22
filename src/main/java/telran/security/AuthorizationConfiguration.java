...
//				.access(new WebExpressionAuthorizationManager("@customWebSecurity.checkOwner(#login)")) // -> v 6.1
						...
						.requestMatchers("/driver/add", "/car/return", "/model/cars", "/car/rent", "/cars/**").hasRole("CLERK")
						.requestMatchers("/driver/cars", "/drivers/car", "/driver", "/driver/*/cars").hasAnyRole("CLERK", "DRIVER")
						.requestMatchers("/drivers/active", "/models/*", "/models/profitable/*/*").hasRole("STATIST")
						.requestMatchers("/*/remove", "/car/add", "/model/add").hasRole("MANAGER")
						.requestMatchers("/account/login", "/account/password", "/car/**", "/car").authenticated()
						.anyRequest().denyAll());
